...
articles. A local
Bridge retains target-bound article versions; the script uploads their images
...
- retained article versions with an explicit `应用 v001` button for each;
- the current result or error message.

It will not inject into a mismatched origin/Space/Thread or click Publish.
Selecting any version explicitly replaces the open editor, including a version
...
For selector development, `pnpm dev` starts at port 8865 and advances to the
next free port automatically. This Vite server is development-only: install the
artifact generated by `pnpm build` for normal testing and use. The Codex plugin
starts the Bridge on demand; the companion discovers its active port in the
fixed loopback range only after `刷新版本` and verifies its health token, so no
...
the prior temporary `fileKey`; changed image bytes, changed width, cancel/close,
or page reload uploads the image again.
